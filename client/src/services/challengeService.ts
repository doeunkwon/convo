...
export async function generateChallenge(userID: string): Promise<Challenge | null> {
  ...
  if (user) {
    ...
    const response = await fetch(`${baseUrl}:1000/generate-challenge?userID=${userID}`, {
...
export async function saveChallengeToServer(
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:1000/save-challenge`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
...
export async function getUserChallenge(userID: string): Promise<Challenge | null> {
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:1000/get-challenge?userID=${userID}`, {
      headers: {
        Authorization: token,
      },
    });
...
export async function deleteChallenge(userID: string): Promise<void> {
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:1000/delete-challenge?userID=${userID}`, {
      method: "DELETE",
      headers: {
        Authorization: token,
      },
