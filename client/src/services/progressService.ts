...
export async function saveProgressToServer(
  ...
  if (user) {
    ...
    const response = await fetch(`${baseUrl}:10000/save-progress`, {
      method: "POST",   
      headers: {
...
export async function updateProgress(userID: string, progress: Progress): Promise<void> {
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:10000/update-progress?userID=${userID}`, {
...
export async function getUserProgress(userID: string): Promise<Progress | null> {
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:10000/get-progress?userID=${userID}`, {
      headers: {
        Authorization: token,
      },
    });
...
export async function deleteProgress(userID: string): Promise<void> {
  ...
  if (user) {
    const token = await user.getIdToken();
    const response = await fetch(`${baseUrl}:10000/delete-progress?userID=${userID}`, {
      method: "DELETE",
      headers: {
        Authorization: token,
      },
