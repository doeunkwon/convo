...
export async function getUserPreference(userID: string): Promise<Preference | null> {
    ...
    if (user) {
      ...
      const response = await fetch(`${baseUrl}:1000/get-preference?userID=${userID}`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  export async function updateUserPreference(userID: string, preference: Preference): Promise<void> {
    ...
    if (user) {
      const token = await user.getIdToken();
      const response = await fetch(`${baseUrl}:1000/update-preference?userID=${userID}`, {
  ...
  export async function saveUserPreference(userID: string, preference: Preference): Promise<void> {
    ...
    if (user) {
      const token = await user.getIdToken();
      const response = await fetch(`${baseUrl}:1000/save-preference`, {
        method: "POST",   
        headers: {
          "Content-Type": "application/json",
          Authorization: token,
  ...
  export async function deleteUserPreference(userID: string): Promise<void> {
    ...
    if (user) {
      const token = await user.getIdToken();
      const response = await fetch(`${baseUrl}:1000/delete-preference?userID=${userID}`, {
        method: "DELETE",
        headers: {
          Authorization: token,
        },
